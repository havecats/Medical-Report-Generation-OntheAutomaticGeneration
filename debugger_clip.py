...
class DebuggerBase:
    ...
    def train(self):
        for epoch_id in range(self.start_epoch, self.args.epochs):
            ...
            # print("[{} - Epoch {}]\ttrain loss:{:.4f}\tval_loss:{:.4f}\tlr:{:.4f}\n".format(self._get_now(),
            #                                                                                 epoch_id,
            #                                                                                 train_loss,
            #                                                                                 val_loss,
            ...
            #                                                                                     0]['lr']))
            # self.writer.write(
            #     "[{} - Epoch {}]\ttrain loss:{:.4f}\tval_loss:{:.4f}\tlr:{:.4f}\n".format(self._get_now(),
            #                                                                               epoch_id,
            #                                                                               train_loss,
            #                                                                               val_loss,
            #                                                                               self.optimizer.param_groups[
...
class LSTMDebugger(DebuggerBase):
    ...
    def _epoch_val(self):
        ...
        #         prev_hidden_states = hidden_states
        #
        #         batch_stop_loss += self.ce_criterion(p_stop.squeeze(), prob_real[:, sentence_index].squeeze()).sum()
        #
        #         for word_index in range(1, captions.shape[2]):
        #             words = self.word_model.forward(topic, context[:, sentence_index, :word_index])
        #             word_mask = (context[:, sentence_index, word_index] > 0).float()
...
if __name__ == '__main__':
    ...
    # Path Argument
    parser.add_argument('--vocab_path', type=str, default='./data/new_data/debugging_vocab.pkl', #debug_vocab.pkl->debugging
    ...
    parser.add_argument('--epochs', type=int, default=50)  # 修改epoch
